...
UK.rows=which(global.data[,2]=="United Kingdom")

# UK.rows=which(global.data[,2]=="UK")
UK.data=global.data[UK.rows,]
UK.data[]

length(UK.data[,1])

day.zero=which(UK.data[,1]=="2020-03-09")
x.start=-which(UK.data[,1]=="2020-03-09")+1
x=x.start:(length(UK.data[,1])-day.zero)
# Check whether the quarantine day 2020-03-09 falls on 0
# data.frame(x,UK.data[,1])
##########################ĘĘ
#Total fits
###################

 # Fitting data from total cases
 total.cases=UK.data[,5]
 ...
 plot(x,UK.data[,5],main="Total number of cases in UK",xlab="Day since lockdown",ylab="Total number of cases",xlim=c(-38,20),ylim=c(0.1,4*max(UK.data[,5],na.rm=T   )),log="")
 ...
 # Fitting data from total deaths
        total.deaths=UK.data[,6]
 data.to.fit=data.frame(x,total.deaths)
 data.fit=summary(nls(total.deaths~a*exp(x/b)+c,data=data.to.fit,start=list(a=10,b=5,c=3),trace = T))
 
 # Plotting
 plot(x,UK.data[,6],main="Total number of deaths in UK",xlab="Day since lockdown",ylab="Total number of deaths",xlim=c(-38,20),ylim=c(0.1,4*max(UK.data[,6],na.rm=T)  ),log="")
