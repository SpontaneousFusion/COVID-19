...
Sweden.rows=which(global.data[,2]=="Sweden")
Sweden.data=global.data[Sweden.rows,]
Sweden.data[,1]

length(Sweden.data[,1])

day.zero=which(Sweden.data[,1]=="2020-03-09")
x.start=-which(Sweden.data[,1]=="2020-03-09")+1
x=x.start:(length(Sweden.data[,1])-day.zero)
# Check whether the quarantine day 2020-03-09 falls on 0
# data.frame(x,Sweden.data[,1])
##########################ĘĘ
#Total fits
###################

 # Fitting data from total cases
 total.cases=Sweden.data[,5]
 ...
 plot(x,Sweden.data[,5],main="Total number of cases in Sweden",xlab="Day since lockdown",ylab="Total number of cases",xlim=c(-38,20),ylim=c(0.1,4*max(Sweden.data[,5],na.rm=T   )),log="")
 ...
 total.deaths=Sweden.data[,6]
 data.to.fit=data.frame(x,total.deaths)
 data.fit=summary(nls(total.deaths~a*exp(x/b)+c,data=data.to.fit,start=list(a=10,b=3,c=3),trace = T))
 
 # Plotting
 plot(x,Sweden.data[,6],main="Total number of deaths in Sweden",xlab="Day since lockdown",ylab="Total number of deaths",xlim=c(-38,20),ylim=c(0.1,4*max(Sweden.data[,6],na.rm=T)  ),log="")
 polygon(c(0,0,12,12),c(-100000,100000,100000,-100000),col=rgb(0,0,0,0.1),border="NA")
 curve( data.fit$coefficients[1,1]*exp(x/data.fit$coefficients[2,1])+ data.fit$coefficients[3,1],add=T,col=2,lwd=2)
 abline(h=data.fit$coefficients[1,1]*exp(12/data.fit$coefficients[2,1])+data.fit$coefficients[3,1],lty=2)
 abline(v=c(0,12),lwd=2,lty=3)
 text( 12,data.fit$coefficients[1,1]*exp(12/data.fit$coefficients[2,1])+data.fit$coefficients[3,1],    round(data.fit$coefficients[1,1]*exp(12/data.fit$coefficients[2,1])+data.fit$coefficients[3,1],0),pos=3,col=1,cex=1.2)
